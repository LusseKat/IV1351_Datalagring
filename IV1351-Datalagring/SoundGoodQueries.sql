...
-- Show number of lessons per month during a specified year
...
-- Show number of lessons per month/type of lesson for specified year
...
-- Retrieve avarage lessons per month over the whole year
SELECT COUNT(*)/12 AS average_lessons 
FROM schedule
WHERE EXTRACT(YEAR FROM timeslot)='2022';

-- Avarage specific lesson per month over the whole year 
...
-- Number of lessons worked by a specifik instructor
...
-- List of ensembles held during next week sorted by genre and weekday
-- tell if full booked, 1-2 seats left
...
--checking all rentals from a certain type
